...
8. **Only after the schema + RLS + types are settled** does the corresponding `apps/<module>` frontend get scaffolded (or, for Team Members, `apps/hub` gets extended). When scaffolding that frontend, use the `department-app-shell` skill (`.claude/skills/department-app-shell/`) for the outer sidebar-nav + content-area shell rather than reinventing it — every department app shares one shell design (collapsible Gravity UI icon sidebar, floating content card), set by `apps/atlas` and `apps/hub` and documented there.
...
3. First department app, only once its DB module has completed the Section 3.1 process end-to-end and is reflected in the Module Tracker. Scaffold its shell with the `department-app-shell` skill (Section 3.1, step 8) rather than designing a new nav/layout from scratch.
4. Repeat per department, in the priority order the DB scoring matrix gives — don't reorder based on frontend convenience alone. Same shell skill applies every time; if this is the third app adopting it, the skill itself flags when the shared parts are worth promoting into `packages/ui-kit`/`packages/config` (Section 4) instead of copying again.
...
- **Do** use the `department-app-shell` skill when scaffolding a new department app's nav/content shell — it's the same collapsible-sidebar-plus-floating-content-card design across every app (`apps/atlas`, `apps/hub`), not a per-app design decision.
...
- ~~Confirm the live Supabase project ID/name before any agent runs a migration~~ — **done, 2026-08-17**: the live project is `matnispbauvvlnbsuzxq` ("research-and-development-webapp"). It was not a clean slate — see `db/MIGRATIONS.md` for what else has run against it and why that history looks the way it does. Both `team-members` and `feedback` migrations (schema + advisor fixes) are applied and advisor-clean. ~~`supabase/functions/*` are written but not yet deployed~~ — **stale as of 2026-09-25**: confirmed live via `list_edge_functions` that most functions under `supabase/functions/` are already deployed to `matnispbauvvlnbsuzxq` (including every Hub/Team-members and Atlas/orders function). `customer-codes-remove` and `salesperson-codes-remove` are the two written-but-not-deployed exceptions found so far — verify against `list_edge_functions` before assuming a given function is live, rather than trusting this note indefinitely, since nothing keeps it in sync automatically.
