...
|---|---|---|
| `team-members` | `apps/hub` | Applied, advisor-clean. Schema + RLS + onboarding/admin extensions live. Edge functions `employee-signup`, `employee-signin`, `invite-employee`, `update-employee`, `update-own-profile`, `upload-employee-avatar` confirmed **deployed and ACTIVE** on `matnispbauvvlnbsuzxq` (2026-09-25 pass, via `list_edge_functions`). Hub's new Settings > Departments/Roles/Apps management screens added 9 more, all deployed and smoke-tested the same pass: `create/update/delete-department`, `create/update/delete-role`, `create/update/delete-app` — no schema change needed, they write to the existing `departments`/`roles`/`apps` tables under their existing `*.manage` permissions. Advisors re-run after deploy: no new findings, only pre-existing ones in unrelated modules. |
...
| `orders` | `apps/atlas` | **Schema (`001`–`008`) + all 10 Edge Functions applied and deployed, advisor-clean, smoke-tested; live on the internal deploy server** (`http://192.168.0.18:3001`) — see `db/MIGRATIONS.md`'s "Orders module — still pending" section and `apps/atlas/README.md`. The ERP pull no longer runs as a Supabase Edge Function/pg_cron job — a real feed pull (~120k rows/~145MB) hit a hard Edge Function compute ceiling twice even after a streaming rewrite, so it now runs as a plain Node script on the deploy server itself (`apps/atlas/scripts/orders-sync.mjs`, system cron, `008`). Unified salesperson/production/shipping/sales order visibility, replacing the standalone `Track JR Orders` tool. First to use a plain data-driven reference table (`stages`) instead of a Postgres enum for a status-like field, deliberately, per its own spec. One auth system throughout (Supabase Auth) — briefly had a second, Clerk-based login for "merchants" before `006` (2026-09-01) corrected that assumption: those are internal territory heads/B2B salespeople, not external customers, so they're just employees rows scoped by `merchant_customer_codes`. `004` adds a workflow layer (structured work requests, milestones, an append-only audit log, and the real named production-escalation chain) replacing the order@/mzpreview@ email relay — every part of it was prototyped and load-tested against the live ERP feed in a local tool before being written as a migration. Pilot scope confirmed: London (customer `34836`) — not yet seeded; separately, Dinesh Choudhary (territory head, 72 real customer codes) is seeded but not yet linked to an account. |
...
- `team-members`'s deploy status is now re-confirmed (see above, 2026-09-25); `feedback`'s
  Edge Function deployment status is still unconfirmed against the live project.
