...
function EmployeeLoginForm() {
  ...
  async function handleSubmit(event: React.FormEvent) {
    ...
    try {
      // Matched against employees.employee_code alone — not Supabase Auth, no phone/email
      // needed. No password, no auth.users row, no session (see
      // supabase/functions/employee-signin). Remembering "this browser is employee X" is
      // just a plain cookie, set below.
      const supabase = getBrowserSupabaseClient();
      const { employeeId } = await employeeSignIn(supabase, { employeeCode });
