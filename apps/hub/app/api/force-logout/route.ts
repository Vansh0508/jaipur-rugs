...
// "authenticated + on /login -> bounce forward" rule immediately redirect right back in —
// an infinite loop. This Route Handler has cookie-write access, so it signs out (clearing
// the session cookie) and *then* redirects, so the next proxy.ts pass sees no session and
// lets the request land on /login cleanly.
